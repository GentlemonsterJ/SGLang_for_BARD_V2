...
from sglang.srt.configs.dsar_vl import DstarVLConfig, DstarVLVisionConfig
...
from sglang.srt.models.dstar import DstarModel
...
class Dstar_VisionMLP(nn.Module):
    def __init__(
        self,
        in_features: int,
        hidden_features: int,
...
class DstarVLVisionPatchEmbed(nn.Module):
...
class Dstar_VisionBlock(nn.Module):
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class DstarVLMoeVisionPatchMerger(nn.Module):
    def __init__(
        self,
        dim: int,
        context_dim: int,
...
class DstarVLMoeVisionModel(nn.Module, RotaryPosMixin):
    def __init__(
        self,
        ...
        self.patch_embed = DstarVLVisionPatchEmbed(config=vision_config)
        ...
        self.merger = DstarVLMoeVisionPatchMerger(
...
class DstarLLMModel(DstarModel):
    def __init__(
        self,
        *,
        config: DstarVLConfig,
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
    ):
...
class DstarVLForConditonalGeneration(nn.Module):
    ...
    def __init__(
        ...
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
        language_model_cls=DstarLLMModel,
    ) -> None:
        super().__init__()
        self.pp_group = get_pp_group()
        ...
        self.visual = DstarVLMoeVisionModel(
        ...
        if language_model_cls is DstarLLMModel:
            self.config: DstarVLConfig = config  # for dstar-vl
        else:
            self.config = config.text_config  # for dstar-omni
    ...
    def get_image_feature(self, items: List[MultimodalDataItem]) -> torch.Tensor:
        # in dstar-vl, last dim is the same
    ...
    def forward(
        ...
        Args:
            ...
            positions: Flattened (concatenated) position ids corresponding to a
                batch.
                **NOTE**: If mrope is enabled (default setting for Dstar-VL
...
EntryClass = DstarVLForConditonalGeneration
