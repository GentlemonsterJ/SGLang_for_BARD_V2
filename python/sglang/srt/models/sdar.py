...
def _normalize_sdar_rope_positions_dtype(
    positions: torch.Tensor, forward_batch: ForwardBatch
) -> torch.Tensor:
    """Keep SDAR RoPE indices aligned with fused KV cache index dtype."""
    out_cache_loc = getattr(forward_batch, "out_cache_loc", None)
    if out_cache_loc is None or positions.dtype == out_cache_loc.dtype:
        return positions
    return positions.to(dtype=out_cache_loc.dtype)
...
class SDARAttention(nn.Module):
    ...
    def forward_prepare_native(self, positions, hidden_states, forward_batch):
        positions = _normalize_sdar_rope_positions_dtype(positions, forward_batch)
