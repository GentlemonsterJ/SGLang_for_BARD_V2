...
class PrefillAdder:
    ...
    def _add_dllm_req(self, req: Req, prefix_len: int):
        # DLLM prompt prefill should advance one response/prompt block at a time.
        # Keep the last prompt chunk partial if needed, but never schedule more
        # than one block per round.
        remaining_len = len(req.fill_ids) - prefix_len
        trunc_len = min(self._get_dllm_remain_tokens(), remaining_len)
