...
def general_mm_embed_routine(
    ...
    **kwargs,
    ...
    def _embed_tokens_preserving_mm_placeholders(
        input_ids: torch.Tensor, cached_mm_input_embeds: Optional[torch.Tensor]
    ) -> torch.Tensor:
        vocab_size = embed_tokens.num_embeddings
        clamped_input_ids = input_ids.clamp(min=0, max=vocab_size - 1)
        input_embeds = embed_tokens(clamped_input_ids)

        if cached_mm_input_embeds is None:
            return input_embeds

        mm_placeholder_mask = (input_ids < 0) | (input_ids >= vocab_size)
        if mm_placeholder_mask.any():
            input_embeds[mm_placeholder_mask] = cached_mm_input_embeds[
                mm_placeholder_mask
    ...
    if not hasattr(language_model, "pp_group") or language_model.pp_group.is_first_rank:
        if (
            ...
            input_embeds = _embed_tokens_preserving_mm_placeholders(
                input_ids, forward_batch.mm_input_embeds
            )
