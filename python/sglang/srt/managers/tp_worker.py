...
class TpModelWorker(BaseTpWorker):
    ...
    def _init_dllm_algorithm(self):
        ...
        from sglang.srt.dllm.verifier import DllmLocalVerifier
        ...
        if self.server_args.dllm_algorithm is not None:
            ...
            self.dllm_verifier = (
                ...
                if self.server_args.dllm_verifier_model_path is not None
                else None
            )
        else:
            self.dllm_algorithm = None
            self.dllm_verifier = None
    ...
    def _forward_batch_generation_dllm(
        ...
        model_worker_batch: Optional[ModelWorkerBatch] = None,
    ) -> GenerationBatchResult:
        if self.dllm_verifier is not None:
            run_result = self.dllm_algorithm.run(
                self.model_runner,
                forward_batch,
                model_worker_batch=model_worker_batch,
                verifier=self.dllm_verifier,
            )
        else:
        ...
        if len(run_result) == 7:
            ...
            verify_accept_map = None
            verify_logprob_map = None
        elif len(run_result) == 4:
            logits_output, next_token_ids, can_run_cuda_graph, step_map = run_result
            step_confidence_map = None
            verify_accept_map = None
            verify_logprob_map = None
        else:
            logits_output, next_token_ids, can_run_cuda_graph = run_result
            step_map = None
            step_confidence_map = None
            verify_accept_map = None
            verify_logprob_map = None
        return GenerationBatchResult(
            ...
            verify_accept_map=verify_accept_map,
            verify_logprob_map=verify_logprob_map,
        )
