...
def _is_dllm_multimodal_cuda_graph_unsupported(
    is_dllm: bool, is_multimodal: bool, forward_batch: ForwardBatch
) -> bool:
    if not (is_dllm and is_multimodal):
        return False
    return (
...
class CudaGraphRunner:
    ...
    def can_run(self, forward_batch: ForwardBatch):
        # Standard CUDA graph replay does not carry multimodal embeddings/MM caches.
        # For DLLM multimodal prefill, replay would silently drop the visual/audio
        # conditioning and behave like a text-only request.
        if _is_dllm_multimodal_cuda_graph_unsupported(
            self.is_dllm, self.model_runner.is_multimodal, forward_batch
        ):
            return False
        ...
        # DLLM may schedule a partial final prompt chunk. CUDA graphs are captured
        # for full blocks only, so partial blocks must fall back to eager forward.
        is_dllm_supported = (
            forward_batch.batch_size * self.num_tokens_per_bs
            == forward_batch.input_ids.numel()
            if self.is_dllm
