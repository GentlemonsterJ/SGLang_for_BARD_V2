...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # Disable KV writes for iterative refinement passes that should reuse
    # an existing prefix cache without mutating it.
    disable_kv_cache_write: bool = False
    ...
    return_step_map: Optional[List[bool]] = None
    return_step_confidence_map: Optional[List[bool]] = None
    ...
    def init_new(
        ...
        ret = cls(
            ...
            return_step_map=batch.return_step_map,
            return_step_confidence_map=batch.return_step_confidence_map,
        ...
        # Override the positions with spec_info. DLLM positions are initialized
        # after extend lengths are available because prompt prefill can end with
        # a partial block.
        if (
        ...
        if ret.forward_mode.is_decode() or ret.forward_mode.is_target_verify():
            ...
            if batch.dllm_config is not None:
                ...
                ret.positions = torch.tensor(
                    [
                        pos
                        for block_offset, extend_len in zip(
                            batch.dllm_block_offsets, batch.extend_seq_lens
                        )
                        for pos in range(block_offset, block_offset + extend_len)
                    ],
        ...
        for batch_idx in range(batch_size):
            ...
            if self.forward_mode.is_decode():
                ...
                if (
                    ...
                    full_mrope_positions = mm_input.mrope_positions
                    required_seq_len = extend_prefix_len + extend_seq_len
                    if full_mrope_positions.shape[1] < required_seq_len:
                        from sglang.srt.managers.mm_utils import (
                            extend_mrope_positions_for_retracted_request,
                        )

                        full_mrope_positions = (
                            extend_mrope_positions_for_retracted_request(
                                full_mrope_positions,
                                required_seq_len - full_mrope_positions.shape[1],
                            )
                        )
                    mrope_positions = full_mrope_positions[
                        :,
