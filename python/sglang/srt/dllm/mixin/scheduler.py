...
class SchedulerDllmMixin:
    def _append_dllm_step_map(self: Scheduler, req: Req, step_map: List[int]) -> None:
        if not step_map:
            return
        if req.customized_info is None:
            req.customized_info = {}
        existing_step_map = req.customized_info.setdefault("step_map", [])
        step_offset = max(existing_step_map, default=0)
        existing_step_map.extend(step + step_offset for step in step_map)

    def _append_dllm_step_confidence_map(
        self: Scheduler, req: Req, step_confidence_map: List[float]
    ) -> None:
        if not step_confidence_map:
            return
        if req.customized_info is None:
            req.customized_info = {}
        req.customized_info.setdefault("step_confidence_map", []).extend(
            step_confidence_map
        )

    def _append_dllm_verify_accept_map(
        self: Scheduler, req: Req, verify_accept_map: List[bool]
    ) -> None:
        if not verify_accept_map:
            return
        if req.customized_info is None:
            req.customized_info = {}
        req.customized_info.setdefault("verify_accept_map", []).extend(
            verify_accept_map
        )

    def _append_dllm_verify_logprob_map(
        self: Scheduler, req: Req, verify_logprob_map: List[float]
    ) -> None:
        if not verify_logprob_map:
            return
        if req.customized_info is None:
            req.customized_info = {}
        req.customized_info.setdefault("verify_logprob_map", []).extend(
            verify_logprob_map
        )

    def _append_dllm_output_logprobs(
        self: Scheduler,
        req: Req,
        ...
        if (
            ...
            or not next_token_ids
            or result.logits_output is None
            or result.logits_output.full_logits is None
        ):
            return

        block_size = self.dllm_config.block_size
        new_tokens = len(next_token_ids)
        block_start = req_idx * block_size
        token_start = block_size - new_tokens
        curr_logits = result.logits_output.full_logits[
            block_start + token_start : block_start + block_size
        ]
        ...
        token_logprobs = (
            torch.log_softmax(curr_logits.float(), dim=-1)
    ...
    def process_batch_result_dllm(
        ...
        if result.next_token_ids:
            ...
            for idx in range(batch.batch_size()):
                ...
                self._append_dllm_output_logprobs(req, result, idx, next_token_ids)
                step_map = (
                    result.step_map[idx]
                    if result.step_map is not None and idx < len(result.step_map)
                    else None
                )
                self._append_dllm_step_map(req, step_map)
                step_confidence_map = (
                    result.step_confidence_map[idx]
                    if result.step_confidence_map is not None
                    and idx < len(result.step_confidence_map)
                    else None
                )
                self._append_dllm_step_confidence_map(req, step_confidence_map)
                verify_accept_map = (
                    result.verify_accept_map[idx]
                    if result.verify_accept_map is not None
                    and idx < len(result.verify_accept_map)
                    else None
                )
                self._append_dllm_verify_accept_map(req, verify_accept_map)
                verify_logprob_map = (
                    result.verify_logprob_map[idx]
                    if result.verify_logprob_map is not None
                    and idx < len(result.verify_logprob_map)
                    else None
                )
                self._append_dllm_verify_logprob_map(req, verify_logprob_map)
