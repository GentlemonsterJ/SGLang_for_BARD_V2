...
from sglang.srt.entrypoints.openai.utils import (
    ...
    process_step_confidence_map_from_ret,
    process_step_map_from_ret,
...
DSTAR_VL_DEFAULT_SYSTEM_PROMPT = "You are a helpful assistant."


def _maybe_prepend_dstar_default_system_message(
    messages: List[Dict[str, Any]],
    model_type: Optional[str],
) -> List[Dict[str, Any]]:
    if model_type != "dstar_vl":
        return messages
    ...
    return [
        {"role": "system", "content": DSTAR_VL_DEFAULT_SYSTEM_PROMPT},
        *messages,
    ]
...
class OpenAIServingChat(OpenAIServingBase):
    ...
    def _convert_to_internal_request(
        ...
        adapted_request = GenerateReqInput(
            **prompt_kwargs,
            ...
            return_logprob=(
                request.logprobs
                or request.return_token_logprobs
            ...
            return_step_map=request.return_step_map,
            return_step_confidence_map=request.return_step_confidence_map,
            rid=request.rid,
    ...
    def _apply_jinja_template(
        ...
        if self.use_dpsk_v32_encoding:
            ...
            messages = _maybe_prepend_dstar_default_system_message(
                messages, self._get_model_type()
            )
            ...
            openai_compatible_messages = _maybe_prepend_dstar_default_system_message(
                openai_compatible_messages, self._get_model_type()
            )
    ...
    async def _generate_chat_stream(
        ...
        try:
            ...
            response_sglext = self._build_response_sglext(content, request)
            if response_sglext is not None:
    ...
    def _build_chat_response(
        ...
        choices = []
        response_sglext = self._build_response_sglext(ret[0], request)
    ...
    def _build_response_sglext(
        ...
        step_map = (
            process_step_map_from_ret(ret_item) if request.return_step_map else None
        )
        step_confidence_map = (
            process_step_confidence_map_from_ret(ret_item)
            if request.return_step_confidence_map
            else None
        )
        token_logprobs = (
        ...
        token_ids = (
            process_token_ids_from_ret(ret_item)
            if request.return_token_ids or request.return_token_logprobs
            else None
        )
        if (
            routed_experts
            ...
            return SglExt(
                ...
                step_map=step_map,
                step_confidence_map=step_confidence_map,
