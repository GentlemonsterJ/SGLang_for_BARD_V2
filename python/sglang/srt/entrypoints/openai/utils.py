...
def process_step_map_from_ret(ret_item: Dict[str, Any]) -> Optional[List[int]]:
    """Process Dstar DLLM unmask step map from a ret item."""
    return ret_item["meta_info"].get("step_map", None)


def process_step_confidence_map_from_ret(
    ret_item: Dict[str, Any]
) -> Optional[List[float]]:
    """Process Dstar DLLM unmask confidence map from a ret item."""
    return ret_item["meta_info"].get("step_confidence_map", None)


def process_token_logprobs_from_ret(
    ret_item: Dict[str, Any]
) -> Optional[List[Optional[float]]]:
    """Process selected-token logprobs from a ret item into a flat list."""
    token_logprobs = ret_item["meta_info"].get("output_token_logprobs", None)
    if token_logprobs is None:
        return None
    return [item[0] if item is not None else None for item in token_logprobs]


def process_token_ids_from_ret(ret_item: Dict[str, Any]) -> Optional[List[int]]:
    """Process selected-token ids from a ret item into a flat list."""
    token_logprobs = ret_item["meta_info"].get("output_token_logprobs", None)
    if token_logprobs is None:
        return None
    return [item[1] for item in token_logprobs if item is not None]
