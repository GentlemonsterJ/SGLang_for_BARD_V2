...
def _get_model_type_label(model_config) -> str:
    ...
    return type(hf_config).__name__


def _get_model_config_attr(model_config, attr: str):
    ...
    if hf_config is not None:
        return getattr(hf_config, attr, None)

    return None


def validate_multimodal_model_pair_compatibility(
    target_model_config,
    peer_model_config,
) -> tuple[str, str]:
    target_model_type = _get_model_type_label(target_model_config)
    peer_model_type = _get_model_type_label(peer_model_config)

    target_is_vlm = bool(
        getattr(target_model_config, "is_image_understandable_model", False)
        or getattr(target_model_config, "is_audio_understandable_model", False)
    )
    peer_is_vlm = bool(
        getattr(peer_model_config, "is_image_understandable_model", False)
        or getattr(peer_model_config, "is_audio_understandable_model", False)
    )

    if not target_is_vlm and not peer_is_vlm:
        return target_model_type, peer_model_type

    if target_is_vlm != peer_is_vlm:
        raise ValueError(
            "Model pairing requires both models to be multimodal or both to be text-only. "
            f"Got target={target_model_type}, peer={peer_model_type}."
        )
    ...
    for attr in compared_attrs:
        target_value = _get_model_config_attr(target_model_config, attr)
        peer_value = _get_model_config_attr(peer_model_config, attr)
        if target_value != peer_value:
            mismatches.append(f"{attr}: target={target_value}, peer={peer_value}")

    if mismatches:
        mismatch_summary = "; ".join(mismatches)
        raise ValueError(
            "Model pairing requires compatible tokenizer/vocab boundaries. "
            f"Got target={target_model_type}, peer={peer_model_type}. "
...
def validate_standalone_multimodal_draft_model_pair(
    target_model_config,
    draft_model_config,
    *,
    enable_multimodal: Optional[bool],
    language_only: bool,
) -> str:
    target_model_type, draft_model_type = validate_multimodal_model_pair_compatibility(
        target_model_config,
        draft_model_config,
    )
    ...
    if language_only:
        raise ValueError(
            "Standalone speculative decoding with multimodal draft/verifier pairs "
            "does not support --language-only."
        )

    if enable_multimodal is False:
        raise ValueError(
            "Standalone speculative decoding with multimodal draft/verifier pairs "
            "requires multimodal processing to stay enabled."
        )
...
class ServerArgs:
    ...
    dllm_verifier_model_path: Optional[str] = None
    dllm_verifier_revision: Optional[str] = None
    dllm_verifier_threshold: float = -5.0
        ...
        if self.speculative_algorithm in ("EAGLE", "EAGLE3", "STANDALONE"):
            ...
            if self.speculative_algorithm == "STANDALONE":
                if self.speculative_draft_model_path is None:
                    raise ValueError(
                        "Standalone speculative decoding requires "
                ...
                self._speculative_draft_model_type = (
                    validate_standalone_multimodal_draft_model_pair(
                ...
                self._speculative_draft_model_config = draft_model_config
    ...
    def _handle_dllm_inference(self):
        ...
        if self.speculative_algorithm is not None:
            raise ValueError(
                "Diffusion LLM inference does not support combining "
                "--dllm-algorithm with --speculative-algorithm."
            )
        ...
        if self.dllm_verifier_model_path is None:
            if self.dllm_algorithm == "Dflash":
                raise ValueError(
                    "Dflash requires --dllm-verifier-model-path to be configured."
                )
            return

        if self.dllm_algorithm != "Dflash":
            raise ValueError(
                "--dllm-verifier-model-path is only supported with --dllm-algorithm Dflash."
            )

        if self.enable_multimodal is False:
            raise ValueError(
                "Diffusion draft plus AR verifier requires multimodal processing to stay enabled."
            )

        if self.language_only:
            raise ValueError(
                "Diffusion draft plus AR verifier does not support --language-only."
            )

        from sglang.srt.configs.model_config import ModelConfig

        main_model_config = self.get_model_config()
        verifier_model_config = ModelConfig.from_server_args(
            self,
            model_path=self.dllm_verifier_model_path,
            model_revision=self.dllm_verifier_revision,
        )
        main_model_type, verifier_model_type = (
            validate_multimodal_model_pair_compatibility(
                main_model_config,
                verifier_model_config,
            )
        )

        if main_model_type != "dstar_vl":
            raise ValueError(
                "Dflash currently only supports dstar_vl as the diffusion draft model. "
                f"Got {main_model_type}."
            )
        if verifier_model_type != "qwen3_vl":
            raise ValueError(
                "Dflash currently only supports qwen3_vl as the AR verifier model. "
                f"Got {verifier_model_type}."
            )

        self._dllm_verifier_model_type = verifier_model_type
        self._dllm_verifier_model_config = verifier_model_config

        if self.enable_mm_global_cache:
            logger.warning(
                "Global multimodal embedding cache is disabled because verifier "
                "scoring must use raw multimodal features."
            )
            self.enable_mm_global_cache = False
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--dllm-verifier-model-path",
            type=str,
            default=ServerArgs.dllm_verifier_model_path,
            help="The path of the AR verifier model used for DLLM diffusion verification.",
        )
        parser.add_argument(
            "--dllm-verifier-revision",
            type=str,
            default=ServerArgs.dllm_verifier_revision,
            help="The specific verifier model revision to use.",
        )
        parser.add_argument(
            "--dllm-verifier-threshold",
            type=float,
            default=ServerArgs.dllm_verifier_threshold,
            help="Minimum verifier logprob required to accept a diffusion token.",
        )
