...
class FlashInferAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        required_workspace_size = None
        if (
            "Qwen2ForCausalLM" in architectures
            or "Qwen3ForCausalLM" in architectures
            or "MiMoForCausalLM" in architectures
            or "Qwen3VLForConditionalGeneration" in architectures
            or "Qwen3VLMoeForConditionalGeneration" in architectures
        ):
            required_workspace_size = 512 * 1024 * 1024
        if "DstarVLForConditonalGeneration" in architectures:
            required_workspace_size = max(
                required_workspace_size or 0, 768 * 1024 * 1024
            )
        if required_workspace_size is not None:
    ...
    def forward_extend(
        ...
        if not self.forward_metadata.use_ragged:
            ...
            if forward_batch.forward_mode.is_dllm_extend():
                # D-LLM denoising uses block-wise bidirectional attention inside
                # the active block rather than left-to-right causal masking.
                causal = False
